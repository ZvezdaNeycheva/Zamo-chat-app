...
export function SingleCannel() {
    ...
    useEffect(() => {
        try {
            ...
            const messagesRef = ref(db, `channels/${idCannel}/messages`);
    ...
    const sendMessage = async () => {
        ...
        const newMessageRef = await push(ref(db, `channels/${idCannel}/messages`), message);
    ...
    const handleEdit = async (mId, newContent) => {
        setEditMessage(mId);
        try {
            const messageRef = ref(db, `channels/${idCannel}/messages/${mId}`);
                ...
                <div className="lg:flex">
                    ...
                    <div className="relative w-full overflow-hidden ">
                        ...
                        <div className="h-[80vh] overflow-scroll p-4 lg:p-6">
                            {/* {fetching Messages} */}
                            <ul className="mb-0">
                                {!idCannel ? <p>Select a channel to start a chat.</p> : null}
                                {idCannel && !messages.length ? <p>The messages in your channel will appear here.</p> : null}
                                {messages.length > 0 &&
