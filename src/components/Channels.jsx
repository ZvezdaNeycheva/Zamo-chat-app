...
export function Channels({groupId}) {
    ...
    useEffect(() => {
      ...
      const handleCreateChannel = async (event) => {
        ...
        try {
          const creatorId = userData?.uid; // Get the UID from the current user object
          const creatorName = userData.username
          const newGroup = await addChannel(groupId, isPublic, creatorName, members, channelName);

          setGroups(prevGroups => ({ ...prevGroups, [newGroup.id]: newGroup }));
          console.log("Group created with privacy setting:", isPrivate);
          setIsModalVisible(false); // Close the modal
          setChannelName(''); // Reset the group name input field
